...
type
  TBreakpoint =     Record
     ...
     Log:           String;  // *not* a text string - a text expression.
     Condition:     String;  // optional expression. If result > 0 then breakpoint will break execution.
  End;
  ...
  public
    { Public declarations }
    Function        CreateBreakpoint(Enabled: Boolean; Line, Statement: DWord; Condition: String): Word;
    Function        ToggleBreakpoint(Line, ListLine: Word; Statement: Byte): Integer;
...
Function  TBreakpointsWindow.CreateBreakpoint(Enabled: Boolean; Line, Statement: DWord; Condition: String): Word;
Var
  F: Word;
Begin
...
begin
  ...
  ToggleBreakpoint(BreakArray[BpNum].Line, ListNum, BreakArray[BpNum].Statement);
  BASinOutput.RepaintBASIC(True);
end;
...
Function TBreakpointsWindow.ToggleBreakpoint(Line, ListLine: Word; Statement: Byte): Integer;
Var
  NewBP, F: Word;
Begin
  Result := 65535;
  If BreakpointsList[Line][Statement] = #0 Then Begin // Does not already exist, so add it
     ...
     If NewBP < 65535 Then Begin
        BreakpointsList[Line][Statement] := Chr(Byte(NewBP));
        Result := NewBP;
     End;
  End Else Begin // Remove it
     ...
     For F := NewBP To 254 Do Begin
        ...
        BreakPointsList[BreakArray[F].Line][BreakArray[F].Statement] := Chr(Ord(BreakPointsList[BreakArray[F].Line][BreakArray[F].Statement]) -1);
     End;
...
begin
  ...
  BreakpointsList[BASinOutput.CursLineNum][BASinOutput.CursStatementNum] := Chr(Byte(NewBP));
