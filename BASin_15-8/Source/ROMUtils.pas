...
Uses Windows, Classes, SysUtils, Clipbrd, Math, Messages;

Type

  TSpectrumError    = Record Modified: Boolean; Address: Word; Desc: String; Notify: Boolean; End;
  TFindResult       = Record LineNum, Position, Address: Word; End;
  TSymbol           = Record sType: Integer; Desc: String; Address: DWord; End;
  ...
  Procedure ROMTrap;
	Procedure ModifyROM;
  Function  GetCurrentStatement(Line: String): String;
	Function  GetEditLine: String;
  Procedure PutEditLine(Tokens: String; Var Mem: Array of Byte);
  Function  TokeniseLine(Line: String; TokeniseStrings: Boolean): String;
  Function  DeTokeniseLine(Line: String; DetokeniseStrings: Boolean): String;
	Function  EditCursorMove(MoveMode: Byte): Byte;
  Function  GetMemoryString(Address, Count: Word; var Mem: Array of Byte): String;
  Procedure DoError(Code: Byte; ErrorText: String);
  ...
  Procedure DeleteEditorLines(Lines: String);
  Procedure DeleteBASICLines(Lines: String);
  Procedure SendLineToEditor(Line: Word; Statement: Byte);
  Procedure MoveSpectrumMemory(StartAddr: Word; Dist: Integer);
  Function  GetVariable(VarName: String; Address: Word): String;
  ...
  Procedure InsertText(Text: String; Posn: Integer);
  Function  GetCursorPos: Word;
  Function  FindNextForward(FindText: String; MatchCase: Boolean): TFindResult;
  Function  FindNextBackward(FindText: String; MatchCase: Boolean): TFindResult;
  Function  ReversePos(SubString: String; Text: String): Integer;
  Procedure ReplaceText(Address: Word; Text: String; Bytes: Word);
  Procedure LOADQuoteQuote(FName: String);
  Function  AutoLine(Tokens: String): String;
  Function  InsertLine(Line: String; Overwrite: Boolean): String;
  ...
  Function  GetClipText(Index: Integer): String;
...
Var
  ...
  Errors: Array[0..43, 1..4] of String =
...
Const
  ...
  AsciiKeywords: Array[0..102] of String = ('SPECTRUM', 'PLAY', 'RND', 'INKEY$', 'PI', 'FN', 'POINT', 'SCREEN$', 'ATTR',
  ...
  ColourNames: Array[0..9] of String = ('Black', 'Blue', 'Red', 'Magenta', 'Green', 'Cyan', 'Yellow', 'White', 'No Change', 'Contrast');
...
Begin
  If Not FileExists(Filename) Then Begin
     MessageBox(BASinOutput.Handle, 'The 48k Spectrum Rom could not be opened.'#13'BASin cannot run without this file,'#13'and will now close.', PChar('Missing ROM File'), MB_OK or MB_ICONWARNING);
     PostMessage(BASinOutput.Handle, WM_CLOSE, 0, 0);
     Result := False;
     Exit;
  End;
...
Function InsertRAMDiskCodes(Text: String): String;
Var
  InString: Boolean;
  TempStr: String;
  Idx, Idx2: Integer;
	...
	Case Registers.PC Of
        ...
        Begin
           ...
           End Else Begin
              If TapeTrapSave Then Begin
                 ...
                 FileBody[3] := Chr(Registers.A);        // flag byte (in A reg)
                 ...
                 If TAPSaveLen <> 0 Then
                    For Loop := 0 to TAPSaveLen-1 Do Begin
                       ...
                       FileBody := FileBody + Chr(SaveByte);
                       ChkSum := ChkSum Xor SaveByte;
                    End;
                 FileBody := FileBody + Chr(ChkSum);     // finally, write the checksum
                 ...
                 FileBody := '  ' + Chr(0) + FileHeader + Chr(ChkSum) + FileBody;
     ...
     $1F55:
        Begin
           // A BREAK trap
				Registers.F := Registers.F Or 1;
           If (DWord(GetKeyState(VK_ESCAPE)) > 1) or WantBreak Then Begin
...
Var
	F: DWord;
Begin
	// Returns the current Edit line as a string of characters.
	Result := '';
	TokenBuffer := '';
	F := Memory[FLAGX];
	If F and 32 = 32 Then
...
Begin
	...
	// Add the terminator string $0D - if in editmode, add an extra $80 later
	Tokens := Tokens+#$0D;
...
Var
  ...
  InString, REMCommand: Boolean;
Begin
  // Converts a string of Sinclair BASIC tokens to plain text
	TempLine := Line;
	// Strip leading spaces
	While Copy(TempLine, 1, 1) = ' ' Do TempLine := Copy(TempLine, 2, 999999);
  // Now iterate through, and test for each char,
	// if > $A4 then replace with string from LUT, and pad with spaces.
	Index := 1;
  LastToken := '';
  InString := False;
  REMCommand := False;
...
Function GetMemoryString(Address, Count: Word; Var Mem: Array of Byte): String;
Begin
  // Grabs a string of bytes from Spectrum Memory and
  // returns it as a string.
  Result := '';
  While Count > 0 Do Begin
     Result := Result + Chr(Mem[Address]);
     Inc(Address);
     Dec(Count);
  End;
End;
...
Var
  ...
  B: Byte;
Begin
  // Puts an error string into the ROM, replacing the error specified.
  ...
  While Memory[VarAddress] <> 128 Do Begin
     Case (Memory[VarAddress] and 224) of
        64:
           Begin
              // String
              List.Add(Chr((Memory[VarAddress] and 31)+96) + '$');
        ...
        160:
           Begin
              ...
              While Memory[VarAddress] and 128 = 0 Do Begin
                 ...
                 Inc(VarAddress);
              End;
              List.Add(TempStr + LowerCase(Chr(Memory[VarAddress] and 127)));
              Inc(VarAddress, 6);
           End;
        128:
           Begin
...
Begin
  ...
  // in a string. If the Address is 0 (address unknown) then the entire VARS memory section
  ...
  While Not Done Do Begin
     ...
     Case (Memory[VarAddress] and 224) of
        64:
           Begin
              // String
              CurName := Chr((Memory[VarAddress] and 31)+96) + '$';
        ...
        160:
           Begin
              ...
              While Memory[VarAddress] and 128 = 0 Do Begin
                 ...
                 Inc(VarAddress);
              End;
              CurName := TempStr + LowerCase(Chr(Memory[VarAddress] and 127));
              If CurName = VarName Then Begin
                 TempByte := Memory[VarAddress+1];
                 TempDWord := GetDWord(@Memory[VarAddress+2]);
                 TempValue := Byte5ToFloat(TempByte, TempDWord);
...
Begin
  ...
  // and the next.
  // then replaces the first char of the line with that number in characters
...
Begin
  ...
  While Addr < VarAddr Do Begin
     ...
     If Modified Then Begin
        ...
        TempStr := Chr(CurLineNum Shr 8)+Chr(CurLineNum And 255)+'  '+TempStr;
...
Var
  ...
  LineStr, TempStr, TempStr2, LogError: String;
  LineNum, Pass, Idx, Idx2, NewLineNum, LineIndex, TempValue: Integer;
