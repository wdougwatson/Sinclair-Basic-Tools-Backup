...
type
  ...
  public
    ...
    NumChars: Integer;
    CurChars: String;        // Curchars holds the current chunk of memory being edited.
    AssignedChars: String;   // Assigned characters - which UDG is assigned to which section of the editing grid
    OnionChars: String;      // As for AssignedChars, but for the onion-skin display
    OnionDown,
    ...
    Function  GetAssignedChars: String;
    ...
    Function  CreateASMChar(Index: Integer): String;
    ...
    Procedure ArrangeChars(SrcChars, DstChars: String);
...
begin
  ...
  For F := 0 To (NumChars * DataWidth * DataHeight) -1 Do
     CurChars := CurChars + Chr(Memory[F+Address]);
  CurChar := 1;
...
Begin
  ...
  FastIMG2.Bmp.Clear(FastDraw.TColorToTFColor(ClBtnFace));
...
Begin
  ...
  If DragOp <> 0 Then Begin
     ...
     If PtInRect(FastIMG1.ClientRect, MousePos) Then Begin
        ...
        // Dragged into the editor area - which char are we over then?
     ...
     End Else Begin
        ...
        If PtInRect(FastIMG3.ClientRect, MousePos) Then Begin
           ...
           // Dragged into the preview area - which char are we over then?
        ...
        End Else Begin
           ...
           If PtInRect(FastIMG6.ClientRect, MousePos) Then Begin
              ...
              // Dragged into the onionskin area - which char are we over then?
...
begin
  ...
  If Not UDGGrabWindow.Cancelled Then Begin
     ...
     // Only multiples of 8 are loaded (round to nearest char),
     ...
     For LPos := Offset to CodeLength +Offset -1 Do
        CurChars := CurChars + Chr(FileArray[LPos]);
...
begin
  If GotClip Then Begin
     ...
     Offset := ((CurChar -1)*DataWidth*DataHeight)+1;
     For Y := 0 To (DataWidth * DataHeight) -1 Do
        CurChars[Offset+Y] := Chr(ClipChar[Y]);
     RepaintChars;
     GetBIGChar(True);
  End;
  MouseDown := False;
...
begin
  UndoBuffer := Chr(CurChar)+CurChars;
  GotUndo := True;
End;
...
Function TUDGWindow.CreateASMChar(Index: Integer): String;
Var
  X, Y, Z, Offset: DWord;
  CurByte: String;
  ByteVal: Byte;
...
begin
  ...
  If DragOp <> 0 Then Begin
     ...
     cW := (DataWidth * 8) + 7;
     cH := DataHeight + 7;
     DragChar := (X Div cW) + (((Y Div cH) * (FastIMG2.Bmp.Width Div cW)));
     If DragChar > NumChars Then
        DragOp := 0
     Else Begin
        DragX := X;
        DragY := Y;
...
begin
  ...
  If PtInRect(ScrollBox1.BoundsRect, NewPos2) And PtInRect(FastIMG2.ClientRect, NewPos) Then Begin
     ...
     CurChar := (X Div cW) + (((Y Div cH) * (FastIMG2.Bmp.Width Div cW)));

     If CurChar <= NumChars Then Begin
  ...
  End Else Begin
     ...
     If PtInRect(FastIMG1.ClientRect, NewPos) Then Begin
        ...
        // Left - drag, assign this char to this grid position
        // Right - drag, copy this char's contents to this grid position
                 ...
                 If AnimSelected <> 0 Then Begin
                    ...
                    If AnimSelected < 0 Then Begin
                       ...
                       If AnimList.Count = 0 Then
                          AnimList.Add(#0#0+Chr(DragChar +1))
                       Else
                          AnimList.Insert(AnimSelected -1, #0#0+Chr(DragChar +1));
                    ...
                    End Else Begin
                       ...
                       End Else Begin
                          ...
                          If AnimSelected = AnimList.Count Then Begin
                             ...
                             AnimList[AnimSelected -1] := #0#0+Chr(DragChar +1);
                             AnimList.Add(#0#0#0);
                             AnimScrollInView(AnimList.Count -1);
                             AnimSelected := 0;
...
Begin
  ...
  StatusBar1.Panels[0].Text := ' Editing '+IntToStr(NumChars) + ' graphic(s) from address '+IntToStr(Address);
  StatusBar1.Repaint;
...
begin
  ...
  If AnimList.Count > 0 Then
     For Y := 0 To AnimList.Count -1 Do
        If Ord(AnimList[Y][3]) >= CurChar Then
           AnimList[Y] := Copy(AnimList[Y], 1, 2)+Chr(Ord(AnimList[Y][3])+1);
...
Begin
  ...
  // Create a temporary storage space for the new rotated char
  ...
  For X := 0 To (DataWidth * 8) -1 Do Begin
     ...
     For Y := DataHeight -1 DownTo 0 Do Begin
        ...
        If Byte(CurChars[Offset]) And Bit <> 0 Then
           NewChar[RowIdx] := Char(Byte(NewChar[RowIdx]) or RowBit);
...
Begin
  ...
  // Create a temporary storage space for the new mirrored char
  ...
  For Y := 0 To DataHeight -1 Do Begin
     For X := 0 To DataWidth -1 Do Begin
        ...
        For H := 7 DownTo 0 Do Begin
           ...
           G := G Shl 1;
        End;
        NewChar[(Y * DataWidth) + ((DataWidth -1) -X) +1] := Chr(FlippedByte);
...
Begin
  ...
  // Create a temporary storage space for the new shifted char
  ...
  For Y := 0 To DataHeight -1 Do Begin
     ...
     For X := DataWidth -1 DownTo 0 Do Begin
        ...
        SavedBit1 := SavedBit2;
        NewChar[(Y * DataWidth) +X +1] := Chr(ByteVal);

        If X = 0 Then
           NewChar[(Y * DataWidth) + DataWidth] := Chr(Byte(NewChar[(Y* DataWidth) + DataWidth]) or SavedBit1);
...
Begin
  ...
  For Y := 0 To DataHeight -1 Do Begin
     ...
     For X := 0 To DataWidth -1 Do Begin
        ...
        If X = DataWidth -1 Then
           NewChar[(Y * DataWidth) + 1] := Chr(Byte(NewChar[(Y* DataWidth) + 1]) or SavedBit1);
...
Begin
  ...
  // Just set all the bytes for this char to 0.

  For Y := 0 To DataHeight -1 Do
     For X := 0 To DataWidth -1 Do
        CurChars[Index + (Y * DataWidth) + X] := Chr(0);
...
Begin
  ...
  // XOR all bytes in the char with 255

  For Y := 0 To DataHeight -1 Do
     For X := 0 To DataWidth -1 Do
        CurChars[Index + (Y * DataWidth) + X] := Chr(Byte(CurChars[Index + (Y * DataWidth) + X]) xor 255);
...
Function TUDGWindow.GetAssignedChars: String;
Var
  cChar: Word;
  Idx, Idx2: Integer;
  Found: Boolean;
  ...
  For Idx := 0 To (Length(AssignedChars) Div 2) -1 Do Begin
     ...
     For Idx2 := 0 To Length(Result) -1 Do
        If GetWord(@Result[(Idx2 * 2) +1]) = cChar Then
           Found := True;
     If Not Found Then
        Result := Result + Chr(cChar And 255) + Chr(cChar Shr 8);
  End;
...
Procedure TUDGWindow.ArrangeChars(SrcChars, DstChars: String);
Var
...
begin
  ...
  // Flip left/right for each char in the grid
...
begin
  ...
  // Flip vertically for each char in the grid
...
begin
  ...
  // First, shift each char left one pixel
  ...
  For Gy := 0 To EditHeight -1 Do
     ...
     For Y := 0 To DataHeight -1 Do Begin
        ...
        For Gx := EditWidth DownTo 1 Do Begin
           ...
           CurChars[X + (Y * DataWidth)] := Chr((Byte(CurChars[X + (Y * DataWidth)]) And 254) + SavedBit);
           SavedBit := SavedBit2;
...
begin
  ...
  // First, shift each char right one pixel
  ...
  For Gy := 0 To EditHeight -1 Do
     ...
     For Y := 0 To DataHeight -1 Do Begin
        ...
        For Gx := 1 To EditWidth Do Begin
           ...
           CurChars[X + (Y * DataWidth)] := Chr((Byte(CurChars[X + (Y * DataWidth)]) And 127) + SavedBit);
           SavedBit := SavedBit2;
  ...
  For Gx := 1 To EditWidth Do
     ...
     For X := 1 To DataWidth Do Begin
        ...
        For Gy := EditHeight -1 DownTo 0 Do Begin
           ...
           CurChars[Index + (DataWidth * (DataHeight -1))] := Chr(SavedByte);
...
Var
  ...
  TempStr, Frame, BlankFrame: String;
  TempDIB: TFastDIB;
...
Begin
  ...
  If AnimList.Count > 0 Then
     For Idx := 0 To AnimList.Count -1 Do Begin
        ...
        If AnimList[Idx][3] = #0 Then Begin
        ...
        End Else Begin
           ...
           For Idx2 := (Ord(TempStr[3]) -1) * DataWidth * DataHeight to ((Ord(TempStr[3])) * DataWidth * DataHeight) -1 Do
...
procedure TUDGWindow.FormKeyPress(Sender: TObject; var Key: Char);
begin

  If Lowercase(Key) = 'a' Then Begin
     AnimList[AnimList.Count -1] := #0#0+Chr(CurChar);
     AnimList.Add(#0#0#0);
     AnimScrollInView(AnimList.Count -1);
     AnimSelected := 0;
     RenderCurrentAnim(False);
