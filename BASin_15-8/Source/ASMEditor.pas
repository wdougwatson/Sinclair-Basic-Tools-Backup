...
type
  ...
  tVisibleLabel = Record X, Y, W, H: Integer; Text: String; End;
  ...
  public
    ...
    VisibleLabels: Array of TVisibleLabel;
    PointingLabel: Integer;
    Procedure SetScrollBoxBounds;
    Procedure RepaintASM(DoPaint: Boolean);
    Procedure NewWord(DIB: TFastDIB; CurWord: String; Var XPos, YPos: Integer; Ink, Paper, Bright: Integer; LineBreak, REMLine, InString: Boolean; CurPos: Integer);
    ...
    Procedure AddVisibleLabel(Caption: String; Xc, Yc: Integer);
  end;
...
Var
  ...
  Ink, Paper, Bright, pInk, pPaper, pBright, CurWordPos, CurWordOrg, LineSelXPos: Integer;
  InSelection, inComment, inString, LineBreak, AllSpaces, LineHasSelection, WholeLineSelected: Boolean;
...
Begin
  ...
  For LineIdx := CurFile^.ViewLine To Min(NumLines -1, CurFile^.ViewLine + PageLen) Do Begin
     ...
     If (LineIdx >= sStartL) and (LineIdx <= sEndL) Then Begin
        ...
        If LineIdx = sStartL Then Begin
           LineSelStart := sStartC;
           LineSelXPos := XPos + (sStartC * 8) -8;
        End Else Begin
           LineSelStart := 1;
           LineSelXPos := XPos;
        End;
        ...
        If (sStartC <> sEndC) or (sStartL <> sEndL) Then Begin
           LineHasSelection := True;
           If (LineSelStart = 1) And(LineSelEnd = Length(AsmText)) Then
              WholeLineSelected := True;
        End;

     End;

     If LineIdx = CurFile^.CursorLine -1 Then WholeLineSelected := False;

     If (Not LineHasSelection) or (Not WholeLineSelected) Then While CurPos <= Length(AsmText) Do Begin
                          ...
                          If (CurChar in ['0'..'9', 'A'..'Z', 'a'..'z', '$', '%', '_', '.', '&']) or InComment or InString Then Begin
                             ...
                             End Else
                                If AllSpaces Then Begin
                                   AllSpaces := False;
                                   If (CurPos >= LineSelStart) and (CurPos <= LineSelEnd) Then
                                      NewWord(FastIMG1.Bmp, CurWord, CurWordOrg, YPos, Ink, Paper, Bright, LineBreak, InComment, InString, CurWordPos);
                                   CurWordOrg := XPos;
                                   AllSpaces := False;
                                   CurWord := '';
...
//                                If Not AllSpaces Then
        ...
        If Length(CurWord) = 1 Then CurWordPos := CurPos;
        Inc(CurPos);
...
//        If Not AllSpaces Then
     ...
     If LineHasSelection Then Begin

        CurPos := LineSelStart;
        CurWord := '';
        CurWordOrg := LineSelXPos;
        CurWordPos := LineSelStart;

        While CurPos <= LineSelEnd Do Begin
           CurWord := CurWord + AsmText[CurPos];
           Inc(CurPos);
        End;

        NewWord(FastIMG1.Bmp, #255+CurWord, CurWordOrg, YPos, Opt_AsmBackground, Opt_AsmForeground, 0, False, False, False, CurWordPos);
        CurWord := '';
...
Procedure TAsmEditorWindow.AddVisibleLabel(Caption: String; Xc, Yc: Integer);
Var
  Len: Integer;
Begin

  Len := Length(VisibleLabels);
  SetLength(VisibleLabels, Len +1);
  With VisibleLabels[Len] Do Begin
     X := Xc;
     Y := Yc;
     W := Length(Caption) * 8;
     H := 8;
...
Procedure TASMEditorWindow.NewWord(DIB: TFastDIB; CurWord: String; Var XPos, YPos: Integer; Ink, Paper, Bright: Integer; LineBreak, REMLine, InString: Boolean; CurPos: Integer);
Var
  NewText, TempStr: String;
  SelStart: Integer;
  Bold, Italic, Selection: Boolean;
Label
  Render;
Begin
  If CurWord = '' Then Exit;
  If CurWord[1] = #255 Then Begin
     Selection := True;
  ...
  If Selection Then Begin
     Ink := Opt_AsmBackground;
     Paper := Opt_AsmForeground;
     Bold := False;
     Italic := False
  End Else If Opt_SyntaxHighlight and not LineBreak Then Begin
     Paper := -1;
           ...
           Chr(Integer(tTLabel)):
              If Opt_AsmHLLabels Then Begin
                 If IsType(TempStr) Then Begin
                    ...
                    AddVisibleLabel(NewText, Xpos, YPos);
                    Goto Render;
                 End;
              End;
...
Begin
  ...
  // Remove a line, its label if necessary, and update the line numbers of other labels accordingly
...
begin
  MouseDown := False;
  If PointingLabel > -1 Then Begin
     For Idx := 0 To ListBox1.Items.Count -1 Do Begin
        If Uppercase(ListBox1.Items[Idx]) = VisibleLabels[PointingLabel].Text Then Begin
           CursorToLine(Integer(ListBox1.Items.Objects[Idx]), 1, '');
           RepaintASM(True);
           MakeCursorVisible;
           Break;
        End;
...
begin
  If MouseDown Then Begin
     cPoint := GetCharPos(X, Y);
     If PointingLabel > -1 Then Begin
        If (cPoint.y <> CurFile^.CursorLine) or (cPoint.x <> CurFile.CursorColumn) Then
           PointingLabel := -1;
     End;
  ...
  End Else Begin
     InLabel := False;
     For Idx := 0 To Length(VisibleLabels) -1 Do Begin
        If (X >= VisibleLabels[Idx].X) and (Y >= VisibleLabels[Idx].Y) And
           (X < VisibleLabels[Idx].W + VisibleLabels[Idx].X) And
           (Y < VisibleLabels[Idx].H + VisibleLabels[Idx].Y) Then Begin
              InLabel := True;
              Break;
        End;
     End;
     If InLabel Then Begin
  ...
  sEnd := SelEnd;
{
  ...
  End Else Begin
     ...
     TempStr := Copy(CurFile^.AsmStrs[sStart.Y -1], 1, sStart.X -1) + Copy(CurFile^.AsmStrs[sEnd.Y -1], sEnd.X +1, 9999999);
