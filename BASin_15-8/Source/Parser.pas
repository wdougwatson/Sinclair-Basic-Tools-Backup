...
Type
  ...
  TParseError =  Record Error, Syntax, Line: String; ErrorCode, Position, PositionEnd, Statement, Linenum: Integer; NowTyping: Set Of 0..255; End;
  ...
  Function  BeautifyBASIC(BASIC: String): String;
  Function  SplitStatements(BASIC: String): TStringlist;
  Function  BuildParseStack(Line: String): TParseStack;
  Function  IsReserved(Token: String): Integer;
  Function  ContainsReserved(Token: String): Integer;
  Function  IndexIsReserved(Token: String): Integer;
  Function  IsKeyword(Token: String): Integer;
  Function  RemoveSpaces(Word: String): String;
  Function  WhatIs(TokenType: Integer): String;

  Function  ParseExprLine(Line: String): TParseError;
  Function  ParseInputLine(Line: String): TParseError;
  Function  GetLineType(Text: String): Integer;
  Function  ParseCodeLine(Line: String; Storable: Boolean): TParseError;
  ...
  Function  ParseGenericType(Var Start: Integer; Key: String; PS: TParseStack; Pri: Integer; ExpectList: Array of Integer; SyntaxList: Array Of String; Return: Integer): Integer;
  ...
  Function  ParseGFXFuncs(Var Start: Integer; Key: String; Params: Array Of String; PS: TParseStack; NumTerms: Integer): Integer;
  ...
  Function  HighlightReserved(Text: String; HighlightSymbols: Boolean): String;
  Procedure ColorTextOut(Canvas: TCanvas; Text: String; X, Y: Integer);
  ...
  Function  GetContents(Thing: TThingSet): String;

Const

  Types: Array[0..32] of String =
  ...
  // Functions
   ...
   AlphaNumerics:   String  = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
   Alphas:          String  = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
   Numerics:        String  = '0123456789';
   Symbols:         String  = '@$!#£%&_{}[]~\|?.()+-/*^"'#39':;,<>=';
   MathSymbols:     String  = '(-+';
...
Var
  ...
  KeyWordNames: Array[1..91] of String;
  KeyWordsIDXTable: Array[1..91] of Integer;
  KeyWordsIDXNames: Array[1..91] of String;
  SyntaxTemplate, ErrorMsg: String;
...
Begin
  ...
  Result.Syntax := Chr(16)+Chr(1)+'LineNumber'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'Statement'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'Expression';
  ...
  Case TempType Of
     ...
     4: Result.Syntax := 'LineNumber ['+Chr(16)+Chr(1)+'Statement'+Chr(16)+Chr(1)+']';
  End;
  If Temptype <> 3 Then Result.LineNum := 0;
  If Result.ErrorCode < 0 Then Begin
     For F := 1 to Length(Result.Syntax) Do Begin
        If Result.Syntax[F] = Chr(1) Then Result.Syntax[F] := Chr(2);
     End;
  End;
...
Begin
  ...
  SyntaxTemplate := SyntaxTemplate + Chr(16)+Chr(4)+' ['+WhatIs(GlobalReturn)+']';
  If TempType <= 0 Then Begin
     If (SyntaxTemplate = '') or (Pos(Chr(1), SyntaxTemplate) = 0) Then
        If GlobalType in NumericTypes Then
           SyntaxTemplate := Chr(16)+Chr(1)+'Numeric Expression'
        Else If GlobalType in StringTypes Then
           SyntaxTemplate := Chr(16)+Chr(1)+'String Expression'
        Else
           SyntaxTemplate := Chr(16)+Chr(1)+'Expression';
     PI := PS.GetItem(Start);
...
begin
  ...
  If Storable Then Begin
     ...
     If (StrToIntDef(LineNum, -1) < 0) Or (StrToIntDef(LineNum, -1) > 9999) Then Begin
        ...
        Result.Statement := 1;
        Result.Syntax := Chr(16)+Chr(2)+'LineNumber'+Chr(16)+Chr(0)+' [ : Statement]';
        Result.ErrorCode := -1;
        Result.Line := Line;
        Result.Position := 1;
  ...
  If Text <> '' Then Begin
     ...
     If Lines.Count > 0 Then Begin
        For F := 0 To Lines.Count -1 Do Begin
           ...
           ErrorMsg := '';
           Start := 1;
           PS := BuildParseStack(Copy(Lines[F], Pos(Chr(255), Lines[F])+1, 999999));
           ...
           If TempType <> SIEol Then Begin
              If TempType > 1000 Then Begin
                 ...
                 If TempType = IsReserved('REM') Then Begin
                    ...
                    Else
                       Result.Syntax := 'REM ['+Chr(16)+Chr(1)+'Comments'+Chr(16)+Chr(0)+']';
                    Result.Error := '';
                    Result.Position := 0;
                 ...
                 End Else Begin
                    ...
                    If LineNum <> '' Then
                       Result.Position := Length(LineNum+' ')+StrToInt(Copy(Lines[F], 1, Pos(Chr(255), Lines[F])-1))+PS.GetItem(1).Position-1
                    Else
                       Result.Position := StrToInt(Copy(Lines[F], 1, Pos(Chr(255), Lines[F])-1))+PS.GetItem(1).Position-1;
                    ...
                    If Storable Then Begin
                       If F = 0 Then
                          Result.Syntax := 'LineNumber'+Chr(16)+Chr(2)+' Statement'+Chr(16)+Chr(0)+' [ : Statement]'
                       Else
                          Result.Syntax := 'LineNumber Statement : '+Chr(16)+Chr(2)+'Statement';
                    End Else Begin
                       If F = 0 Then
                          Result.Syntax := Chr(16)+Chr(2)+'Statement'+Chr(16)+Chr(0)+' [ : Statement]'
                       Else
                          Result.Syntax := 'Statement : '+Chr(16)+Chr(2)+'Statement';
                    End;
                    PS.Free;
                    Exit;
                 End;
           ...
           If TempType <> SIStatement Then Begin
              If LineNum <> '' Then
                 Result.Position := Length(LineNum+' ')+StrToInt(Copy(Lines[F], 1, Pos(Chr(255), Lines[F])-1))+PI.Position-1
              Else
                 Result.Position := StrToInt(Copy(Lines[F], 1, Pos(Chr(255), Lines[F])-1))+PI.Position-1;
              ...
              If TempType <= 0 Then Begin
                 ...
                 If Copy(Lines[F], Length(Lines[F]), 1) = Chr(255) Then Result.Error := 'Expecting a Statement';
                 Result.Syntax := SyntaxTemplate;
              ...
              End Else Begin
                 ...
                 If Storable Then Begin
                    If F = 0 Then
                       Result.Syntax := 'LineNumber'+Chr(16)+Chr(1)+' Statement'+Chr(16)+Chr(0)+' [ : Statement]'
                    Else
                       Result.Syntax := 'LineNumber Statement : '+Chr(16)+Chr(1)+'Statement';
                 End Else Begin
                    If F = 0 Then
                       Result.Syntax := Chr(16)+Chr(1)+'Statement'+Chr(16)+Chr(0)+' [ : Statement]'
                    Else
                       Result.Syntax := 'Statement : '+Chr(16)+Chr(1)+'Statement';
                 End;
...
Var
  Idx: Integer;
Const
  WhatIsTypes: Array[0..47] of String =
...
Begin
  ...
  For F := 1 To Length(BASIC) Do Begin
     ...
     If (BASIC[F] <> ':') or ((BASIC[F] = ':') and InString) Then Begin
        Work := Work + BASIC[F];
     End Else Begin
        Result.Add(IntToStr(Start)+Chr(255)+Work);
        Start := F +1;
        Work := '';
     End;
  End;
...
Var
  ...
  WordType: Integer;
  S, Word: String;
  S2: Char; IsBinary, SciFlag: Boolean;
...
// *************************** Remove Spaces ******************************
// * Simply strips all space chars (32) out of the string, and returns    *
// * That string.                                                         *
// ************************************************************************
  ...
  While Not Finished Do Begin
     ...
     If CurItem = SIOpenBrace Then Begin
        ...
        If (PrevType in VarTypes) or (PrevType in StringTypes) Then Begin
           ...
           End Else NextType := SINumVar;
           If NextType = SINumVar Then CurItem := ParseNumSubs(Start, PS) Else CurItem := ParseStrSubs(Start, PS, NextType);
              ...
              SIAdd:
                    Begin
                       If ((CurItem in StringTypes) and (PrevTerm in NumericTypes)) or
                          ...
                          Exit;
                       End Else Begin
                          If CurItem in StringTypes Then Result := SIStrExpr Else Result := SINumExpr;
                          PrevType := Result;
                       End;
                    End;
...
Begin
  ...
  SyntaxTemplate := 'NumVar('+Chr(16)+Chr(1)+'Subscripts'+Chr(16)+Chr(0)+')';
  Expect := SINumExpr;
  Finished := False;
  While Not Finished Do Begin
     Case Expect Of
        SINumExpr:
           Begin
              SyntaxTemplate := 'NumVar('+Chr(16)+Chr(1)+'Subscript '+Chr(16)+Chr(0)+'[, Subscript] )';
              TempStart := Start;
        ...
        SIComma:
           Begin
              SyntaxTemplate := 'NumVar(Subscript ['+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Subscript] '+Chr(16)+Chr(1)+')';
              TempType := PS.GetItemType(Start);
     ...
     Case Expect Of
           ...
           Begin
              If VarType <> SIStrLiteral Then Begin
                 If VarType = SIStrVarSubs Then
                    SyntaxTemplate := 'String ('+Chr(16)+Chr(1)+'Start'+Chr(16)+Chr(0)+' [ '+Chr(16)+Chr(1)+'TO'+Chr(16)+Chr(0)+' End] )'
                 Else
                    SyntaxTemplate := 'StrVar ('+Chr(16)+Chr(1)+'Subscript'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'Start'+Chr(16)+Chr(0)+' [, '+Chr(16)+Chr(1)+'TO'+Chr(16)+Chr(0)+' End] )';
              End Else
                 SyntaxTemplate := 'StrLiteral ('+Chr(16)+Chr(1)+'Start'+Chr(16)+Chr(0)+' [ '+Chr(16)+Chr(1)+'TO'+Chr(16)+Chr(0)+' End] )';
              TempStart := Start;
        ...
        SIComma:
           Begin
              If VarType <> SIStrLiteral Then Begin
                 If VarType = SIStrVarSubs Then
                    SyntaxTemplate := 'String (Start ['+Chr(16)+Chr(1)+' TO '+Chr(16)+Chr(0)+'End]'+Chr(16)+Chr(1)+' )'
                 Else
                    SyntaxTemplate := 'StrVar (Subscript/Start ['+Chr(16)+Chr(1)+', TO '+Chr(16)+Chr(0)+'End]'+Chr(16)+Chr(1)+' )';
              End Else
                 SyntaxTemplate := 'StrLiteral (Start ['+Chr(16)+Chr(1)+' TO '+Chr(16)+Chr(0)+'End]'+Chr(16)+Chr(1)+' )';
              TempType := PS.GetItemType(Start);
              If TempType = SIComma Then Begin
  ...
  If GotTo Then Begin
     ...
     If GotStart Then Begin
        If VarType = SIStrLiteral Then
           SyntaxTemplate := 'StrLiteral (Start TO ['+Chr(16)+Chr(1)+'End'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+')'
        Else If GotSubs then
           SyntaxTemplate := 'StrVar (Subscripts, Start TO ['+Chr(16)+Chr(1)+'End'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+')'
        Else If VarType <> SIStrVarSubs Then
           SyntaxTemplate := 'StrVar (Start TO ['+Chr(16)+Chr(1)+'End'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+')'
        Else
           SyntaxTemplate := 'String (Start TO ['+Chr(16)+Chr(1)+'End'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+')';
        ...
        If TempType <> SICloseBrace Then Begin
           ...
           End Else Begin
              ...
              End Else Begin
                 ...
                 If VarType = SIStrLiteral Then
                    SyntaxTemplate := 'StrLiteral (Start TO End '+Chr(16)+Chr(1)+')'
                 Else If GotSubs then
                    SyntaxTemplate := 'StrVar (Subscripts, Start TO End '+Chr(16)+Chr(1)+')'
                 Else If VarType <> SIStrVarSubs Then
                    SyntaxTemplate := 'StrVar (Start TO End '+Chr(16)+Chr(1)+')'
                 Else
                    SyntaxTemplate := 'String (Start TO End '+Chr(16)+Chr(1)+')';
                 Temptype := PS.GetItemType(Start);
                 If TempType <> SICloseBrace Then Begin
                    If TempType <> SIEol Then Result := -1 Else Result := 0;
     ...
     End Else Begin
        If VarType = SIStrLiteral Then
           SyntaxTemplate := 'StrLiteral ( TO '+Chr(16)+Chr(1)+' End '+Chr(16)+Chr(0)+')'
        Else If GotSubs then
           SyntaxTemplate := 'StrVar (Subscripts, TO '+Chr(16)+Chr(1)+' End '+Chr(16)+Chr(0)+')'
        Else If VarType <> SIStrVarSubs Then
           SyntaxTemplate := 'StrVar ( TO '+Chr(16)+Chr(1)+' End '+Chr(16)+Chr(0)+')'
        Else
           SyntaxTemplate := 'String ( TO '+Chr(16)+Chr(1)+' End '+Chr(16)+Chr(0)+')';
        ...
        End Else Begin
           ...
           End Else Begin
              //now expecting a close bracket.
              If VarType = SIStrLiteral Then
                 SyntaxTemplate := 'StrLiteral ( TO End '+Chr(16)+Chr(1)+')'
              Else If GotSubs then
                 SyntaxTemplate := 'StrVar (Subscripts, TO End '+Chr(16)+Chr(1)+')'
              Else If VarType <> SIStrVarSubs Then
                 SyntaxTemplate := 'StrVar ( TO End '+Chr(16)+Chr(1)+')'
              Else
                 SyntaxTemplate := 'String ( TO End '+Chr(16)+Chr(1)+')';
              Temptype := PS.GetItemType(Start);
              If TempType <> SICloseBrace Then Begin
                 If TempType <> SIEol Then Result := -1 Else Result := 0;
                 ErrorMsg := 'Expecting a <Close Bracket>.';
...
{CLS}       1007: Begin GlobalReturn := SIStatement;  Result := ParseGenericType(Start, 'CLS ', PS, 0, [SIEol], [Chr(16)+Chr(0)+'[No Parameters]'], SIStatement); End;
{CONTINUE}  1008: Begin GlobalReturn := SIStatement;  Result := ParseGenericType(Start, 'CONTINUE ', PS, 0, [SIEol], [Chr(16)+Chr(0)+'[No Parameters]'], SIStatement); End;
...
{NEW}       1028: Begin GlobalReturn := SIStatement;  Result := ParseGenericType(Start, 'NEW ', PS, 0, [SIEol], [Chr(16)+Chr(0)+'[No Parameters]'], SIStatement); End;
...
{RETURN}    1041: Begin GlobalReturn := SIStatement;  Result := ParseGenericType(Start, 'RETURN ', PS, 0, [SIEol], [Chr(16)+Chr(0)+'[No Parameters]'], SIStatement); End;
...
{SPECTRUM}  1044: Begin GlobalReturn := SIStatement;  Result := ParseGenericType(Start, 'SPECTRUM ', PS, 0, [SIEol], [Chr(16)+Chr(0)+'[No Parameters]'], SIStatement); End;
{STOP}      1045: Begin GlobalReturn := SIStatement;  Result := ParseGenericType(Start, 'STOP ', PS, 0, [SIEol], [Chr(16)+Chr(0)+'[No Parameters]'], SIStatement); End;
...
{VAL$}      1090: Begin GlobalReturn := SIStrExpr;    Result := ParseGenericType(Start, 'VAL$ ', PS, 11, [SIStrExpr], ['String'], SIStrExpr); End;
  ...
  If result > 0 Then Begin
     For F := 1 to Length(SyntaxTemplate) Do Begin
        ...
        If SyntaxTemplate[F] = Chr(2) Then SyntaxTemplate[F] := Chr(0);
        If (SyntaxTemplate[F] = Chr(1)) and (Not InOptional) Then SyntaxTemplate[F] := Chr(0);
     End;
     If SyntaxBackUp <> '' Then If Pos(Chr(16)+Chr(1), SyntaxTemplate) < 1 Then SyntaxTemplate := SyntaxBackup;
  ...
  While (ExpectPos < High(ExpectList)) and Not Error Do Begin
     ...
     If ExpectPos <= High(SyntaxList) Then Begin
        If (Optional = ExpectPos-1) and (PS.GetItemType(Start) = SIEol) Then Begin
           If Copy(SyntaxTemplate, Length(SyntaxTemplate)-2, 2) = ' '+Chr(16) Then
           ...
           LateOption := True;
        End;
        SyntaxTemplate := SyntaxTemplate + Chr(16)+Chr(1)+SyntaxList[ExpectPos];
     End;
     If ExpectPos < High(ExpectList) Then Begin
        SyntaxTemplate := SyntaxTemplate + Chr(16)+Chr(0);
     ...
     Else
        Begin
           ...
           If TempType <> Expect Then Begin
              If TempType <> SIEol Then Result := -1 Else Result := 0;
              S := WhatIs(Expect);
              If Pos(S, Symbols) <> 0 Then S := ' '+Chr(39)+S+Chr(39)+' ';
...
Begin
  ...
  SyntaxTemplate := 'FN '+Chr(16)+Chr(1)+'Var'+Chr(16)+Chr(0)+' ( Params )';
  TempType := PS.GetItemType(Start);
              ...
              Case Expect Of
                 SIAnyExpr:
                    Begin
                       ...
                       If NumParams > 0 Then
                          SyntaxTemplate := 'FN Var ( Param, '+Chr(16)+Chr(1)+'Param'+Chr(16)+Chr(0)+' )'
                       Else
                          SyntaxTemplate := 'FN Var ( ['+Chr(16)+Chr(1)+'Params'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+')';
                       TempStart := Start;
                 ...
                 SIComma:
                    Begin
                       ...
                       SyntaxTemplate := 'FN Var ( Param ['+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Param]'+Chr(16)+Chr(1)+' )';
                       TempType := PS.GetItemType(Start);
                       If TempType = SIComma Then Begin
                          Expect := SIAnyExpr;
                          Inc(Start);
                       End Else If TempType = SICloseBrace Then Begin
                          GlobalExpect := [SICloseBrace];
                          Finished := True;
                          SyntaxTemplate := 'FN Var ( Params'+Chr(16)+Chr(1)+' )';
                          Inc(Start);
                       End Else Begin
                          If Temptype <> SIEol Then Result := -1 Else Result := SIUnknown;
                          //error - Expecting , or )
...
Begin
  ...
  SyntaxTemplate := 'CAT ['+Chr(16)+Chr(1)+'!/#/Filespec'+Chr(16)+Chr(0)+']';
  Temptype := PS.GetItemType(Start);
  ...
  If TempType = Ord('#') Then Begin
     Inc(Start);
     SyntaxTemplate := 'CAT #'+Chr(16)+Chr(1)+'Channel '+Chr(16)+Chr(0)+'[, FileSpec]';
     TempStart := Start;
     ...
     Channel := True;
     TempType := PS.GetItemType(Start);
     SyntaxTemplate := 'CAT #Channel ['+Chr(16)+Chr(1)+', FileSpec'+Chr(16)+Chr(0)+']';
     If TempType = SIComma Then Begin
        SyntaxTemplate := 'CAT #Channel, '+Chr(16)+Chr(1)+'FileSpec'+Chr(16)+Chr(0);
        Inc(Start);
        TempStart := Start;
        ...
        End Else Begin
           If Not (TempType in StringTypes) Then Begin
              Result := -1; //error - expecting string type
...
Begin
  RAMDiskOp := False;
  SyntaxTemplate := 'ERASE ['+Chr(16)+Chr(1)+'!'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+'Filename';
  Temptype := PS.GetItemType(Start);
  GlobalExpect := [Ord('!'), SIStrExpr];
  If TempType = Ord('!') then Begin
     SyntaxTemplate := 'ERASE ! '+Chr(16)+Chr(1)+'Filename';
     RAMDiskOp := True;
     Inc(Start);
  End;
  Temptype := GetExprType(Start, PS, 0, SIStrExpr, False);
  ...
  End Else Begin
     If Not (TempType in StringTypes) Then Begin
        Result := -1; //error - expecting string
        If RAMDiskOp Then
           ErrorMsg := 'Expecting a String Expression.'
        Else
           ErrorMsg := 'Expecting a "!" or String Expression.';
...
Begin
  RAMDiskOp := False;
  SyntaxTemplate := 'MERGE ['+Chr(16)+Chr(1)+'!'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+'Filename';
  Temptype := PS.GetItemType(Start);
  GlobalExpect := [Ord('!'), SIStrExpr];
  If TempType = Ord('!') then Begin
     SyntaxTemplate := 'ERASE ! '+Chr(16)+Chr(1)+'Filename';
...
Begin
  ...
  Repeat
     SyntaxTemplate := Key+'['+Chr(16)+Chr(1)+'ColourItems'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1);
     If NumTerms = 4 Then Begin OptionalThird := True; TempStart := 3; End Else TempStart := NumTerms;
     For F := 1 to TempStart Do Begin
        If F = 2 Then SyntaxTemplate := SyntaxTemplate + Chr(16)+Chr(0);
        If F <> TempStart Then
           SyntaxTemplate := SyntaxTemplate + Params[F-1]+Chr(16)+Chr(0)+', '
        Else Begin
     ...
     If TempType = SIColourItem Then Begin
        SyntaxTemplate := Key+'ColourItem'+Chr(16)+Chr(1)+' Separator'+Chr(16)+Chr(0)+'...';
        TempType := PS.GetItemType(Start);
...
Begin
  ...
  Repeat
     If Read Then
        SyntaxTemplate := Key + Chr(16)+Chr(1)+' Var'+Chr(16)+Chr(0)+' [,Var]'
     Else
        SyntaxTemplate := Key + Chr(16)+Chr(1)+' Expression'+Chr(16)+Chr(0)+' [, Expression]';
     TempStart := Start;
     ...
     If Read Then
        SyntaxTemplate := Key + 'Var ['+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Var]'
     Else
        SyntaxTemplate := Key + 'Expression ['+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Expression]';
     LastType := TempType;
     Temptype := PS.GetItemtype(Start);
...
Begin
  SyntaxTemplate := 'DIM '+Chr(16)+Chr(1)+'Var'+Chr(16)+Chr(0)+' ( Dimensions )';
  TempType := PS.GetItemType(Start);
  ...
  End Else Begin
     Inc(Start);
     SyntaxTemplate := 'DIM Var'+Chr(16)+Chr(1)+' ('+Chr(16)+Chr(0)+' Dimension )';
     TempType := PS.GetItemType(Start);
     If TempType <> SIOpenBrace Then Begin
     ...
     End Else Begin
        ...
        Repeat
           ...
           SyntaxTemplate := 'DIM Var ( '+Chr(16)+Chr(1)+'Dimension'+Chr(16)+Chr(0)+' [, Dimension] )';
           TempStart := Start;
           ...
           End Else Begin
              If Not(TempType in NumericTypes) Then Begin
                 ...
                 Exit;
              End Else Begin
                 SyntaxTemplate := 'DIM Var ( Dimension ['+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Dimension] '+Chr(16)+Chr(1)+')';
                 TempType := PS.GetItemType(Start);
                 GLobalExpect := [SIComma, SICloseBrace];
                 If TempType <> SIComma Then Begin
                    If TempType <> SICloseBrace Then Begin
                    ...
                    End Else Begin
                       ...
                       GlobalExpect := [SICloseBrace];
                       If PS.GetItemType(Start) = SIEol Then Begin
                          SyntaxTemplate := 'DIM Var ( Dimensions'+Chr(16)+Chr(1)+' )';
                          Result := SIStatement;
                          Exit;
                       End;
                    End;
...
Var
  TempStart, TempType: Integer;
Begin
  SyntaxTemplate := 'COPY ['+Chr(16)+Chr(1)+'EXP'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'Filename'+Chr(16)+Chr(0)+']';
  TempType := PS.GetItemType(Start);
  GlobalExpect := [IsReserved('EXP')+1000, SIStrExpr];
  If TempType = IsReserved('EXP')+1000 Then Begin
     Inc(Start);
     SyntaxTemplate := 'COPY EXP ['+Chr(16)+Chr(1)+'INVERSE'+Chr(16)+Chr(0)+']';
     TempType := PS.GetItemType(Start);
     ...
     If TempType <> IsReserved('INVERSE')+1000 Then Begin
        ...
        Exit;
     End Else Begin
        SyntaxTemplate := 'COPY EXP '+Chr(16)+Chr(1)+'INVERSE';
        Inc(Start);
        If PS.GetItemType(Start) <> SIEol Then Begin
           Result := -1;
  ...
  End Else Begin
     ...
     End Else Begin
        TempType := PS.GetItemType(Start);
        SyntaxTemplate := 'COPY Filename '+Chr(16)+Chr(1)+'TO '+Chr(16)+Chr(0)+'Filename/SCREEN$/LPRINT';
        ...
        End Else Begin
           Inc(Start);
           TempType := PS.GetItemType(Start);
           SyntaxTemplate := 'COPY Filename TO '+Chr(16)+Chr(1)+'Filename/SCREEN$/LPRINT';
           If TempType = SIEol Then Begin
              Result := SIUnknown;
           ...
           End Else If (TempType = IsReserved('SCREEN$')+1000) or (TempType = IsReserved('LPRINT')+1000) Then Begin
              If TempType = IsReserved('SCREEN$')+1000 Then Begin
                 SyntaxTemplate := 'COPY Filename TO '+Chr(16)+Chr(1)+'SCREEN$';
              End Else Begin
                 SyntaxTemplate := 'COPY Filename TO '+Chr(16)+Chr(1)+'LPRINT';
              End;
              Inc(Start);
              If PS.GetItemType(Start) <> SIEol Then Begin
                 Result := -1;
           ...
           End Else Begin
              ...
              End Else Begin
                 If Not (TempType In StringTypes) Then Begin
                    ...
                    Exit;
                 End Else Begin
                    SyntaxTemplate := 'COPY Filename TO '+Chr(16)+Chr(1)+'Filename';
                    Result := SIStatement;
                 End;
              End;
           End;
...
Begin
  Channel := False;
  SyntaxTemplate := 'LIST ['+Chr(16)+Chr(1)+'#'+Chr(16)+Chr(0)+'Channel,]['+Chr(16)+Chr(1)+'Line No'+Chr(16)+Chr(0)+']';
  If PS.GetItemType(Start) = Ord('#') Then Begin
     SyntaxTemplate := 'LIST #'+Chr(16)+Chr(1)+'Channel '+Chr(16)+Chr(0)+'[, Line No]';
     Inc(Start);
     TempStart := Start;
     TempType := GetExprType(Start, PS, 0, SINumExpr, False);
     If TempType <= 0 Then Begin
     ...
     End Else Begin
        ...
        End Else Begin
           Temptype := PS.GetItemType(Start);
           If TempType = SIEol Then Begin
              SyntaxTemplate := 'LIST #Channel ['+Chr(16)+Chr(1)+', '+Chr(16)+Chr(0)+' Line No]';
              Result := SIStatement;
              Exit;
           End Else Begin
              SyntaxTemplate := 'LIST #Channel'+Chr(16)+Chr(1)+', '+Chr(16)+Chr(0)+' Line No';
  ...
  End Else Begin
     ...
     End;
  End;
  If Channel Then SyntaxTemplate := 'LIST #Channel, '+Chr(16)+Chr(1)+'Line No' Else SyntaxTemplate := 'LIST '+Chr(16)+Chr(1)+'Line No';
  TempStart := Start;
  TempType := GetExprType(Start, PS, 0, SINumExpr, False);
  If TempType <= 0 Then Begin
     Result := TempType;
...
Begin
  TempType := PS.GetItemType(Start);
  SyntaxTemplate := 'FORMAT '+Chr(16)+Chr(1)+'LINE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'LPRINT'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'Drive';
  If TempType = SIEol Then Begin
     Result := SIUnknown;
  ...
  End Else If TempType = IsReserved('LINE')+1000 Then Begin
     Inc(Start);
     SyntaxTemplate := 'FORMAT LINE '+Chr(16)+Chr(1)+'Baud Rate';
     TempStart := Start;
     Temptype := GetExprType(Start, PS, 0, SINumExpr, False);
     If TempType <= 0 Then Begin
        Result := TempType;
...
Begin
  TempType := PS.GetItemType(Start);
  SyntaxTemplate := 'LET '+Chr(16)+Chr(1)+'Var'+Chr(16)+Chr(0)+' = Expression';
  If Not(TempType in VarTypes) Then Begin
  ...
  End Else Begin
     ...
     If VarType = SINumVar Then
        SyntaxTemplate := 'LET Var'+Chr(16)+Chr(1)+' = '+Chr(16)+Chr(0)+'Numeric'
     Else
        SyntaxTemplate := 'LET Var$'+Chr(16)+Chr(1)+' = '+Chr(16)+Chr(0)+'String';
     ...
     End Else Begin
        If VarType = SINumVar Then
           SyntaxTemplate := 'LET Var = '+Chr(16)+Chr(1)+'Numeric'
        Else
           SyntaxTemplate := 'LET Var$ = '+Chr(16)+Chr(1)+'String';
        Inc(Start);
        TempStart := Start;
        If VarType = SINumVar Then
           Temptype := GetExprType(Start, PS, 0, SINumExpr, False)
  ...
  Repeat
     If Expect = SIAnyExpr Then Begin
        If (Input and GotParam) or Not Input Then
           SyntaxTemplate := Key+'['+Chr(16)+Chr(1)+Item+'Item/Separator'+Chr(16)+Chr(0)+']'
        Else
           SyntaxTemplate := Key+Chr(16)+Chr(1)+Item+'Item/Separator';
        If INPUT and (PS.GetItemType(Start) = SIOpenBrace) Then Begin // A bracket here is a PRINT system for the prompt.
           SyntaxTemplate := Key+'('+Chr(16)+Chr(1)+'Prompt Sequence'+Chr(16)+Chr(0)+')';
        ...
        If PS.GetItemType(Start) = Ord('#') Then Begin
           If InBrace Then Begin
              ...
              SyntaxTemplate := Key +'('+Chr(16)+Chr(2)+'PRINT Items'+Chr(16)+Chr(0)+') [Seperator]';
              GlobalPos := Start;
              Exit;
           End;
           SyntaxTemplate := Key+'#'+Chr(16)+Chr(1)+'Channel'+Chr(16)+Chr(0)+' [Separator]';
           Inc(Start);
           TempStart := Start;
           TempType := GetExprType(Start, PS, 0, SINumExpr, False);
           If Temptype <= 0 Then Begin
           ...
           End Else Begin
              If Not (TempType in NumericTypes) Then Begin
                 Result := -1; //error - expecting numeric type
                 ErrorMsg := 'Expecting a Numeric Expression.';
                 SyntaxTemplate := Key+'#'+Chr(16)+Chr(2)+'Channel'+Chr(16)+Chr(0)+' [Separator]';
                 GlobalPos := Start;
                 Start := TempStart;
                 Exit;
              End Else Begin
        ...
        End Else Begin
           ...
           If TempType = IsReserved('LINE')+1000 Then Begin
              ...
              End Else Begin
                 Inc(Start);
                 SyntaxTemplate := Key+'LINE '+Chr(16)+Chr(1)+'Variable'+Chr(16)+Chr(0)+' [Separator]';
                 TempType := PS.GetItemType(Start);
        ...
        End;
     End Else Begin
        SyntaxTemplate := Key+Item+'Item ['+Chr(16)+Chr(1)+'Separator'+Chr(16)+Chr(0)+']';
        TempType := PS.GetItemType(Start);
...
Begin
  //First, expect a num/str SL Var.
  SyntaxTemplate := 'DEF FN '+Chr(16)+Chr(1)+'Var'+Chr(16)+Chr(0)+' ( Params ) = Expression';
  TempType := PS.GetItemType(Start);
  If TempType in [4, 6] Then Begin
     ...
     End Else Begin
        ...
        VarType := 'Var$';
     End;
     SyntaxTemplate := 'DEF FN '+VarType+' '+Chr(16)+Chr(1)+' ( '+Chr(16)+Chr(0)+'Params ) = '+ExprType;
     //Next, an Open Bracket.
     Inc(Start);
     If PS.GetItemType(Start) = SIOpenBrace Then Begin
        //Next, expecting either a Close Bracket or any variable.
        SyntaxTemplate := 'DEF FN '+VarType+' ( ['+Chr(16)+Chr(1)+'Params'+Chr(16)+Chr(0)+']'+Chr(16)+Chr(1)+' ) '+Chr(16)+Chr(0)+'= '+ExprType;
        Inc(Start);
        NumParams := 0;
        If PS.GetItemType(Start) = SICloseBrace Then Begin
           Inc(Start);
           ...
           While Not Finished Do Begin
              Case Expect Of
                 SIAnyVar:
                    Begin
                       If NumParams > 0 Then
                          SyntaxTemplate := 'DEF FN '+VarType+' ( Var, '+Chr(16)+Chr(1)+'Var'+Chr(16)+Chr(0)+' ) = '+ExprType
                       Else
                          SyntaxTemplate := 'DEF FN '+VarType+' ( ['+Chr(16)+Chr(1)+'Params'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+' ) '+Chr(16)+Chr(0)+'= '+ExprType;
                       TempType := PS.GetItemType(Start);
                 ...
                 SIComma:
                    Begin
                       SyntaxTemplate := 'DEF FN '+VarType+' ( Var ['+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Var]'+Chr(16)+Chr(1)+' ) '+Chr(16)+Chr(0)+'= '+ExprType;
                       TempType := PS.GetItemType(Start);
                       If TempType = SIComma Then Begin
                          Expect := SIAnyVar;
                          Inc(Start);
  ...
  End;
  If NumParams > 0 Then
     SyntaxTemplate := 'DEF FN '+VarType+' ( Params ) '+Chr(16)+Chr(1)+'= '+Chr(16)+Chr(0)+ExprType
  Else
     SyntaxTemplate := 'DEF FN '+VarType+' ()'+Chr(16)+Chr(1)+' = '+Chr(16)+Chr(0)+ExprType;
  If PS.GetItemType(Start) <> SIEquals Then Begin
     If PS.GetItemType(Start) <> SIEol Then Result := -1 Else Result := SIUnknown;
     //error - expecting =
     ErrorMsg := 'Expecting an '+Chr(39)+'='+Chr(39)+' symbol.';
     Exit;
  End Else Begin
     If NumParams > 0 Then
        SyntaxTemplate := 'DEF FN '+VarType+' ( Params ) = '+Chr(16)+Chr(1)+ExprType
     Else
        SyntaxTemplate := 'DEF FN '+VarType+' () = '+Chr(16)+Chr(1)+ExprType;
     Inc(Start);
     TempStart := Start;
...
Begin
  ...
  Result := SIStatement;
  TempStart := Start;
  SyntaxTemplate := Key+'['+Chr(16)+Chr(1)+'!'+Chr(16)+Chr(0)+'] '+Chr(16)+Chr(1)+'Filename '+Chr(16)+Chr(0);
  If SAVE Then
     SyntaxTemplate := SyntaxTemplate + '[LINE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'SCREEN$]'
  Else
     SyntaxTemplate := SyntaxTemplate + '[DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'SCREEN$]';
  // First an optional "!" mark.

  Temptype := PS.GetItemType(Start);
  GlobalExpect := [Ord('!'), SIStrExpr];
  If TempType = Ord('!') then Begin
     SyntaxTemplate := Key+'! '+Chr(16)+Chr(1)+'Filename '+Chr(16)+Chr(0);
     If SAVE Then
        SyntaxTemplate := SyntaxTemplate + '[LINE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'SCREEN$]'
     Else
        SyntaxTemplate := SyntaxTemplate + '[DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(0)+'SCREEN$]';
  ...
  If RAMDiskOp Then
     SyntaxTemplate := Key+'! Filename ['+Chr(16)+Chr(1)
  Else
     SyntaxTemplate := Key+'Filename ['+Chr(16)+Chr(1);

  If SAVE Then
     SyntaxTemplate := SyntaxTemplate + 'LINE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'SCREEN$'+Chr(16)+Chr(0)+']'
  Else
     SyntaxTemplate := SyntaxTemplate + 'DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'SCREEN$'+Chr(16)+Chr(0)+']';
  TempType := PS.GetItemtype(Start);
  If TempType = SIEol Then Begin
     Result := SIStatement;
     Exit;
  End Else Begin
     SyntaxTemplate := Key+'Filename '+Chr(16)+Chr(1);
     If SAVE Then
        SyntaxTemplate := SyntaxTemplate + 'LINE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'SCREEN$'
     Else
     SyntaxTemplate := SyntaxTemplate + 'DATA'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'CODE'+Chr(16)+Chr(0)+'/'+Chr(16)+Chr(1)+'SCREEN$';

     If TempType = IsReserved('DATA')+1000 Then Begin
        Inc(Start);
        SyntaxTemplate := Key+'Filename DATA '+Chr(16)+Chr(1)+'Var'+Chr(16)+Chr(0)+'()';
        If PS.GetItemType(Start) In VarTypes Then Begin
           Inc(Start);
           SyntaxTemplate := Key+'Filename DATA Var'+Chr(16)+Chr(1)+'('+Chr(16)+Chr(0)+')';
           If PS.GetItemType(Start) = SIOpenBrace Then Begin
              Inc(Start);
              SyntaxTemplate := Key+'Filename DATA Var('+Chr(16)+Chr(1)+')';
              If PS.GetItemType(Start) = SICloseBrace Then Begin
                 Inc(Start);
              End Else Begin
     ...
     End Else If Temptype = IsReserved('LINE')+1000 Then Begin
        If Not SAVE Then Begin
           Result := -1;
           ErrorMsg := 'Can'+Chr(39)+'t LOAD with LINE command.';
           Exit;
        End;
        SyntaxTemplate := Key+'Filename LINE '+Chr(16)+Chr(1)+'Number';
        Inc(Start);
        TempStart := Start;
        Temptype := GetExprType(Start, PS, 0, SINumExpr, False);
        If TempType <= 0 Then Begin
     ...
     End Else If Temptype = IsReserved('SCREEN$')+1000 Then Begin
        Inc(Start);
        If Start<PS.NumItems -1 Then SyntaxTemplate := Key+'Filename '+Chr(16)+Chr(2)+'SCREEN$' Else SyntaxTemplate := Key+'Filename SCREEN$';
     End Else If TempType = IsReserved('CODE')+1000 Then Begin
        If Save Then SyntaxTemplate := Key+'Filename CODE '+Chr(16)+Chr(1)+'Address'+Chr(16)+Chr(0)+', Bytes'
        Else SyntaxTemplate := Key+'Filename CODE ['+Chr(16)+Chr(1)+'Address'+Chr(16)+Chr(0)+'] [, Bytes]';
        Inc(Start);
        If (PS.GetItemType(Start) = SIEol) Then begin
           If Not SAVE Then Begin
              Result := SIStatement;
        ...
        End Else Begin
           ...
           End Else Begin
              ...
              End Else Begin
                 TempType := PS.GetItemType(Start);
                 If Save Then SyntaxTemplate := Key+'Filename CODE Address'+Chr(16)+Chr(1)+','+Chr(16)+Chr(0)+' Bytes'
                 Else SyntaxTemplate := Key+'Filename CODE Address ['+Chr(16)+Chr(1)+', '+Chr(16)+Chr(0)+'Bytes]';
                 If (TempType = SIEol) Then Begin
                    If Not Save Then Begin
                       Result := SIStatement;
                       Exit;
                 ...
                 End Else Begin
                    ...
                    End Else Begin
                       Inc(Start);
                       SyntaxTemplate := Key+'Filename CODE Address, '+Chr(16)+Chr(1)+'Bytes';
                       TempStart := Start;
                       TempType := GetExprType(Start, PS, 0, SINumExpr, False);
                       If TempType <= 0 Then Begin
                          Result := Temptype;
     ...
     If NewText <> '' Then Begin
        If NewText = UpperCase(NewText) Then Begin
           If Not InString and (IsReserved(NewText) <> 0) and Not REM_Found Then Begin
              ...
              Result := Result+Chr(25)+Chr(1)+NewText+Chr(25)+Chr(0);
           End Else Begin
  ...
  If HighlightSymbols Then Begin
     ...
     For F := 1 to length(Result) Do Begin
        ...
        If (Text = ',') or (Text = ';') or (Text = '#') or (Text = ':') Then Begin
           NewText := NewText+Chr(25)+Chr(1)+Text+Chr(25)+Chr(0);
        End Else Begin
  ...
  For F := 0 To Stack.NumItems -1 Do Begin
     ...
     If S.ItemType < 0 Then Result.Add('ERR:'+IntToStr(-S.ItemType)) Else
        If S.ItemType < 32 Then Result.Add(Types[S.Itemtype]) else
           If S.ItemType < 1000 Then Result.Add(Chr(S.ItemType)) Else
...
Begin
  ...
  SyntaxTemplate := 'CAT ['+Chr(16)+Chr(1)+'!/#/Filespec/EXP'+Chr(16)+Chr(0)+']';
  Temptype := PS.GetItemType(Start);
  If TempType = SIEol Then Begin Result := SIStatement; Exit; End;
  ...
  If TempType = Ord('#') Then Begin
     Inc(Start);
     SyntaxTemplate := 'CAT #'+Chr(16)+Chr(1)+'Channel '+Chr(16)+Chr(0)+'[, FileSpec/EXP]';
     TempStart := Start;
     GlobalExpect := [SINumExpr];
     Temptype := GetExprType(Start, PS, 0, SINumExpr, False);
     If TempType <= 0 Then Begin
     ...
     Channel := True;
     TempType := PS.GetItemType(Start);
     SyntaxTemplate := 'CAT #Channel ['+Chr(16)+Chr(1)+', FileSpec/EXP'+Chr(16)+Chr(0)+']';
     If TempType = SIComma Then Begin
        SyntaxTemplate := 'CAT #Channel, '+Chr(16)+Chr(1)+'FileSpec'+Chr(16)+Chr(0)+' [EXP]';
        Inc(Start);
        TempStart := Start;
        GlobalExpect := [SIStrExpr];
        Temptype := GetExprType(Start, PS, 0, SIStrExpr, False);
        ...
        End;
        StringThere := True;
        SyntaxTemplate := 'CAT #Channel, FileSpec ['+Chr(16)+Chr(1)+'EXP'+Chr(16)+Chr(0)+']';
     End Else If TempType <> SIEol Then Begin
        SyntaxTemplate := 'CAT #Channel ['+Chr(16)+Chr(1)+'EXP'+Chr(16)+Chr(0)+']';
     End Else Begin
        Result := SIStatement;
        Exit;
     End;
  End Else Begin
     ...
     If TempType <> rEXP Then Begin
        ...
        End;
        StringThere := True;
        SyntaxTemplate := 'CAT FileSpec ['+Chr(16)+Chr(1)+'EXP'+Chr(16)+Chr(0)+']';
     End Else Begin
  ...
  TempType := PS.GetItemType(Start);
  If TempType = SIEol Then Begin
     SyntaxTemplate := SyntaxTemplate + '['+Chr(16)+Chr(1)+'EXP'+Chr(16)+Chr(0)+']';
     Result := SIStatement;
     Exit;
  End Else If TempType = rEXP Then Begin
     SyntaxTemplate := SyntaxTemplate + Chr(16)+Chr(1)+'EXP';
     Inc(Start);
     TempType := PS.GetItemType(Start);
     If TempType <> SIEol Then Begin
        Result := -1;
