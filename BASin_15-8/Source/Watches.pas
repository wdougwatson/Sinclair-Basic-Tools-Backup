...
type
  TWatch = record
     ...
     wVarName: String;           // The variable name to watch.
     wAddress: Word;             // The Address to watch,if necessary.
     wSize: Byte;                // The size of the result (byte, word, DWord, custom - 1,2,3,4 .. 38 etc)
     LastResult: String;         // Used to determine if the expression has changed.
     Expression: String;         // The Expression to evaluate, or the variable name
...
Function TWatchWindow.CreateWatch(Enabled: Boolean; WatchType, wSize, wTypeIndex: Byte; wAddress: Word; Expression, wVarName: String; CanBreak: Boolean): Byte;
Var
  F: Byte;
Begin
  For F := 0 To 255 Do
